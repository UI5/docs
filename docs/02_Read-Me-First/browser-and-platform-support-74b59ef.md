...
The following tables give a general overview of the browsers, platforms, and reference devices supported by the main OpenUI5 libraries. Combinations not included in the table may still function but are not part of the supported scope. Before you report an issue, make sure that it can be reproduced on one of the browser and platform combinations listed. There are certain known device-browser combinations that lead to visual degradations. For more information, see [Visual Degradations](visual-degradations-f08f296.md).
