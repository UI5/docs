...
-   Option 1 \(recommended\): Using `.less` files with OpenUI5 CLI

    Use `.less` files, organized as shown above, with native CSS syntax for maximum compatibility. For build setup and configuration, see [OpenUI5 CLI](https://ui5.github.io/cli/stable/).
...
-   Use only `library.source.less` files when working with OpenUI5 CLI \(recommended\)

-   Use only `library.css` files when not using OpenUI5 CLI build tooling

-   Never mix both file types in the same theme folder when using OpenUI5 CLI
...
For a complete list of built-in themes provided by OpenUI5, see [Available Themes](available-themes-da0d2e7.md).
