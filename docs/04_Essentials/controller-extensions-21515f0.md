...
OpenUI5 offers two distinct approaches to implement controller extensions, both configurable through the `manifest.json`:
...
> 
> Alternatively, you can specify your controller extension modules in OpenUI5's module syntax, in which case you can omit the suffix. For example, `module:my/sample/Extension` maps to a file named `my/sample/Extension.js`.
> 
...
However, there's an important exception for the core controller lifecycle methods `onInit`, `onExit`, `onBeforeRendering`, and `onAfterRendering`. For these special methods, OpenUI5 ensures your custom application methods are called *in addition to* \(not instead of\) the standard methods:
...
Object mixin controller extensions must be implemented as plain JavaScript objects, not as `sap.ui.core.mvc.Controller` subclasses. By default, OpenUI5 expects controller files to have the `.controller` suffix, so your extension files must follow this convention. However, you can also use OpenUI5's module syntax \(prefixed with `module:`\), which allows you to omit the suffix. For example, `module:my/sample/Extension` maps to a file named `my/sample/Extension.js`.
